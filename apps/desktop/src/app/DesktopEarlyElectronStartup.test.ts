...
describe("DesktopEarlyElectronStartup", () => {
  ...
  it("reads the persisted linux password-store preference before Electron is ready", () => {
    const preference = resolveEarlyLinuxPasswordStorePreference({
      env: { T3TODO_HOME: "/home/user/.t3-test" },
  ...
  it("keeps implicit development state under ~/.t3todo/dev when T3TODO_HOME is unset", () => {
    const preference = resolveEarlyLinuxPasswordStorePreference({
      env: {
        VITE_DEV_SERVER_URL: "http://127.0.0.1:5173",
      },
      ...
      joinPath,
      readFileString: (path) => {
        assert.equal(path, "/home/user/.t3todo/dev/desktop-settings.json");
  ...
  it("treats whitespace-only T3TODO_HOME as unconfigured in development", () => {
    const preference = resolveEarlyLinuxPasswordStorePreference({
      env: {
        T3TODO_HOME: "   ",
        VITE_DEV_SERVER_URL: "http://127.0.0.1:5173",
