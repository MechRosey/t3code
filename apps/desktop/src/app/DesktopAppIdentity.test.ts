...
describe("DesktopAppIdentity", () => {
  it.effect("keeps using the legacy userData path when it already exists", () =>
    withIdentity(
      Effect.gen(function* () {
        ...
        assert.equal(userDataPath, "/Users/alice/Library/Application Support/t3todo");
      }),
  ...
  it.effect("preserves failures while inspecting the legacy userData path", () => {
    const legacyPath = "/Users/alice/Library/Application Support/t3todo";
  ...
  it.effect("configures app identity from the environment commit override", () => {
    ...
    return withIdentity(
      Effect.gen(function* () {
        ...
        assert.deepEqual(calls.setName, ["T3 Todo (Alpha)"]);
        assert.equal(calls.setAboutPanelOptions[0]?.applicationName, "T3 Todo (Alpha)");
