...
describe("parseIssue", () => {
  it.each(issueIds)("round-trips fixture %s to the skill's save form", (id) => {
  ...
  it("discards lines before the frontmatter fence, matching the skill's reader", () => {
  ...
  it("strips a leading BOM and keeps the round-trip BOM-free", () => {
    const bytes = readFixture("roundtrip/bom/e20d1-status-target/e20d1-status-target.md");
  ...
  it("normalizes a CRLF body to LF while the frontmatter stays CRLF", () => {
    const bytes = readFixture("roundtrip/crlf-body/e20d1-status-target/e20d1-status-target.md");
    const text = bytes.toString("utf8");
  ...
  it("tolerates unknown frontmatter fields on read and drops them on serialize", () => {
  ...
  it("keeps colour and epic fields through the round-trip", () => {
  ...
  it("dual-reads a retired status into its canonical name without rewriting the body", () => {
    const raw = [
      "---",
      "id: abc12-retired",
