...
const loadBefore = (id: string) => {
  const bytes = NodeFS.readFileSync(findMarkerPath("board-before", id));
  return { bytes, parsed: parseIssue(bytes.toString("utf8"), id) };
};

const goldenAfter = (verb: string, id: string): Buffer =>
  readFixture(`after-${verb}/${id}/${id}.md`);
...
describe("status mutation equivalence", () => {
  ...
  it("matches the skill's status byte output for an issue carrying colour and epic", () => {
    const bytes = readFixture("roundtrip/colour-epic/4b42b-epic-carrier/4b42b-epic-carrier.md");
    ...
    expectBytes(
      ...
      readFixture("after-special-colour-epic-status/4b42b-epic-carrier/4b42b-epic-carrier.md"),
    );
  });

  it("normalizes retired status input to the canonical name", () => {
  ...
  it("refuses closing a parent with open direct children unless forced", () => {
    ...
    expect(() => assertStatusChildrenGuard("done", ["done", "cancelled"], undefined)).not.toThrow();
    expect(() => assertStatusChildrenGuard("doing", ["backlog"], undefined)).not.toThrow();
  });
});

describe("comment mutation equivalence", () => {
  it("matches the skill's comment byte output", () => {
    const { parsed } = loadBefore("762b5-comment-target");
    const golden = goldenAfter("comment", "762b5-comment-target").toString("utf8");
    const stamp = /- \*\*(\d{4}-\d{2}-\d{2} \d{2}:\d{2})\*\*/.exec(golden)?.[1];
    expect(stamp).toBeDefined();
    const next = applyComment(parsed, { text: "A comment lands here.", by: "tester" }, stamp!, NOW);
    expectBytes(Buffer.from(serializeIssue(next)), Buffer.from(golden, "utf8"));
  });

  it("appends under an existing Log without a second header", () => {
    const { parsed } = loadBefore("762b5-comment-target");
  ...
  it("creates a Log section when the body has none", () => {
    const { parsed } = loadBefore("55041-tag-target");
...
describe("tag mutation equivalence", () => {
  it("matches the skill's tag-add byte output", () => {
    const { parsed } = loadBefore("55041-tag-target");
    const next = applyTag(parsed, { tag: "beta" }, NOW);
    expectBytes(Buffer.from(serializeIssue(next)), goldenAfter("tag", "55041-tag-target"));
  });

  it("matches the skill's tag-remove byte output", () => {
    const { parsed } = loadBefore("64eb7-tag-remove-target");
    const next = applyTag(parsed, { tag: "alpha", remove: true }, NOW);
    expectBytes(
      Buffer.from(serializeIssue(next)),
      goldenAfter("tag-remove", "64eb7-tag-remove-target"),
    );
  });

  it("is idempotent when adding an existing tag", () => {
    const { parsed } = loadBefore("64eb7-tag-remove-target");
...
describe("link mutation equivalence", () => {
  it("matches the skill's link-add byte output with the canonical target id", () => {
    const { parsed } = loadBefore("67e0c-link-target");
    ...
    expectBytes(Buffer.from(serializeIssue(next)), goldenAfter("link", "67e0c-link-target"));
  });

  it("matches the skill's link-remove byte output", () => {
    const { parsed } = loadBefore("4fbef-link-remove-target");
    const next = applyLink(
      parsed,
    ...
    expectBytes(
      Buffer.from(serializeIssue(next)),
      goldenAfter("link-remove", "4fbef-link-remove-target"),
    );
  });

  it("stores the canonical target id when the caller typed a short prefix", () => {
    const { parsed } = loadBefore("67e0c-link-target");
    const next = applyLink(
      parsed,
  ...
  it("does not duplicate an edge already stored under a different spelling", () => {
    const { parsed } = loadBefore("4fbef-link-remove-target");
    const next = applyLink(
      parsed,
  ...
  it("rejects a link type outside the vocabulary", () => {
    const { parsed } = loadBefore("67e0c-link-target");
    expect(() =>
      applyLink(
        parsed,
...
describe("rollup mutation equivalence", () => {
  it("matches the skill's rollup byte output on the parent", () => {
    const parent = loadBefore("e594b-rollup-child");
    const child = loadBefore("0fef4-rollup-grandchild");
    const next = applyRollup(
      parent.parsed,
      { id: child.parsed.fm.id, title: child.parsed.fm.title },
      { status: "done", text: "Resolved cleanly" },
      NOW,
    );
    expectBytes(Buffer.from(serializeIssue(next)), goldenAfter("rollup", "e594b-rollup-child"));
  });

  it("replaces an existing child resolution line instead of appending a second one", () => {
    const parent = loadBefore("e594b-rollup-child");
  ...
  it("inserts a Child resolutions section before an existing Log", () => {
    const commented = applyComment(
      loadBefore("762b5-comment-target").parsed,
      { text: "context" },
      "2026-09-20 12:00",
      NOW,
    );
