...
it.layer(NodeServices.layer)("dev-runner", (it) => {
  ...
  describe("createDevRunnerEnv", () => {
    ...
    it.effect("leaves the shared home implicit and disables browser auto-open", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.T3TODO_HOME, undefined);
    ...
    it.effect("supports explicit typed overrides", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.T3TODO_HOME, path.resolve("/tmp/custom-t3"));
    ...
    it.effect("uses custom t3Home when provided", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.T3TODO_HOME, path.resolve("/tmp/my-t3"));
      }),
    );
  ...
  describe("runDevRunnerWithInput", () => {
    ...
    // Base-dir precedence (--home-dir > worktree .t3 > ambient T3TODO_HOME)
    ...
    it.effect("ignores an ambient T3TODO_HOME when no home is resolved", () =>
      Effect.gen(function* () {
        const env = yield* createDevRunnerEnv({
          mode: "dev",
          baseEnv: { T3TODO_HOME: "/home/user/.t3" },
          serverOffset: 0,
          webOffset: 0,
          t3Home: undefined,
          browser: undefined,
    ...
    describe("t3 home precedence", () => {
      ...
      const spawnedHome = (input: {
        ...
        Effect.gen(function* () {
          ...
          yield* runDevRunnerWithInput({ ...devServerInput, t3Home: input.t3Home }).pipe(
            ...
            Effect.provideService(
              HostProcessEnvironment,
              input.ambientHome === undefined ? {} : { T3TODO_HOME: input.ambientHome },
            ),
          );

          return captured?.T3TODO_HOME;
        });
      ...
      it.effect("prefers the worktree .t3 over an ambient T3TODO_HOME", () =>
        Effect.gen(function* () {
          const path = yield* Path.Path;
          const root = yield* makeWorktree;
          const home = yield* spawnedHome({
      ...
      it.effect("falls back to an ambient T3TODO_HOME outside a worktree", () =>
        Effect.gen(function* () {
          const path = yield* Path.Path;
          const home = yield* spawnedHome({
            t3Home: undefined,
