...
stdout/stderr at `~/.t3todo/ssh-launch/<state>/server.log`.
...
- SSH-managed launch persistence: `~/.t3todo/ssh-launch/<state>/server.log`
...
`<home>/userdata/logs/server.trace.ndjson` (so `~/.t3todo/userdata/...` by default, or
...
worktree uses `~/.t3todo/dev/logs/server.trace.ndjson`.
...
```bash
TRACE_FILE="${T3TODO_HOME:-$HOME/.t3todo}/userdata/logs/server.trace.ndjson"
```
...
```bash
TRACE_FILE="$HOME/.t3todo/dev/logs/server.trace.ndjson"
```
